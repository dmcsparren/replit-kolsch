...
export default function InventoryLevels() {
  const [sortBy, setSortBy] = useState<"critical" | "category" | "name">("critical");
  ...
  const getForecastBadge = (forecast: string) => {
    if (forecast === "Order soon") {
      ...
      return <Badge variant="outline" className="bg-yellow-100 text-yellow-700 border-0">{forecast}</Badge>;
    } else {
      return <Badge variant="outline" className="bg-green-100 text-green-700 border-0">{forecast}</Badge>;
    }
  };
        ...
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button 
              size="sm" 
              variant="ghost" 
              className={sortBy === "critical" ? "bg-primary bg-opacity-10 text-primary" : ""} 
            ...
            <Button 
              size="sm" 
              variant="ghost" 
              className={sortBy === "category" ? "bg-primary bg-opacity-10 text-primary" : ""} 
            ...
            <Button 
              size="sm" 
              variant="ghost" 
              className={sortBy === "name" ? "bg-primary bg-opacity-10 text-primary" : ""} 
              ...
              <tbody>
                {sortedItems().slice(0, 5).map((item) => (
                  <tr key={item.id} className="border-b border-neutral-100">
                    ...
                    <td className="py-3 pr-2 text-sm text-neutral-600">{item.category}</td>
                    <td className="py-3 pr-2">
                      <span className="text-sm font-mono">{item.currentQuantity} {item.unit}</span>
                    </td>
                    <td className="py-3 pr-2">
