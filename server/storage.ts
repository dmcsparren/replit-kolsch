...
export class DatabaseStorage implements IStorage {
  ...
  async initializeData() {
    try {
      ...
      if (userCount.length > 0 && userCount[0].count !== undefined) {
      ...
      await this.createInventoryItem({
        ...
        notes: "Floral, citrus aroma. Good for American ales.",
        supplier: "Northwest Hop Farms",
        imageUrl: null
      });
      ...
      await this.createInventoryItem({
        ...
        threshold: 50,
        costPerUnit: 2.49,
        expirationDate: new Date(new Date().setMonth(new Date().getMonth() + 12)).toISOString(),
        notes: "Base malt for many beer styles.",
      ...
      await this.createEquipment({
        ...
        capacity: 500,
        unitOfMeasure: "L",
        acquisitionDate: new Date(new Date().setFullYear(new Date().getFullYear() - 2)).toISOString(),
        lastMaintenanceDate: new Date(new Date().setMonth(new Date().getMonth() - 1)).toISOString(),
      ...
      await this.createEquipment({
        ...
        capacity: 600,
        unitOfMeasure: "L",
        acquisitionDate: new Date(new Date().setFullYear(new Date().getFullYear() - 1)).toISOString(),
        lastMaintenanceDate: new Date(new Date().setMonth(new Date().getMonth() - 2)).toISOString(),
      ...
      const now = new Date();
      const startDate = new Date(now.setHours(10, 0, 0, 0));
